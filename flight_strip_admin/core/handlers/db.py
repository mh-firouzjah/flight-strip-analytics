...
def reconcile_journeys(df: pl.DataFrame):
    ...
    continuations = (
        boundary_df.join(db_boundary_df, on=["callsign", "dep", "des"], how="inner", suffix="_old")
        .filter(pl.col("point1time") < pl.col("sector_end_old"))
        .with_columns(pl.col("journey_id_old").alias("journey_id"))
        # .select(db_boundary_df.columns)  # drop *_old
    )
    ...
    df = (
        ...
        .select(db_boundary_df.columns)  # drop *_cont
    )
